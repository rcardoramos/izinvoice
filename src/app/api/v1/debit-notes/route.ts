...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const expectedPrefix = isFactura ? 'FD' : 'BD';
    if (!serie.startsWith(expectedPrefix)) {
      return NextResponse.json(
        { statusCode: 400, message: `Serie inválida. Las notas de débito para ${isFactura ? 'Facturas' : 'Boletas'} deben comenzar con '${expectedPrefix}'.` },
        { status: 400 }
      );
    }
    ...
    const seriesIndex = seriesList.findIndex(
      (s: any) => s.company_id === ctx.company.id && s.doc_type === '08' && s.serie === serie
    );

    if (seriesIndex === -1) {
      return NextResponse.json(
        { statusCode: 400, message: `La serie '${serie}' no está registrada para Notas de Débito.` },
        { status: 400 }
      );
    }
    ...
    const documentRecord = {
      ...
      payload: {
        ...
        motivoCodigo: motivoCodigo || '01', // Catálogo 10: Interés por mora
        motivoDescripcion: motivoDescripcion || 'INTERESES POR MORA',
        totals: {
    ...
    SunatMockService.generateAndStoreXml(docId, '08', serie, nextCorrelativo, documentRecord.payload);
    ...
    FileDb.insert('invoice_notes', {
      ...
      motivo_descripcion: motivoDescripcion || 'AJUSTE DE VALOR',
    });

    if (isFactura) {
      // Synchronous path (sendBill)
      const sunatResult = SunatMockService.sendBillMock('08', serie, nextCorrelativo, total);
      documentRecord.status = sunatResult.accepted ? 'accepted' : 'rejected';
      FileDb.insert('documents', documentRecord);
      
      SunatMockService.generateAndStoreCdr(docId, '08', serie, nextCorrelativo, sunatResult.accepted, sunatResult.errorMessage || '');
      ...
      const message = sunatResult.accepted
        ? `Nota de Débito ${serie}-${nextCorrelativo} aceptada con éxito por SUNAT.`
        : `Nota de Débito ${serie}-${nextCorrelativo} rechazada: ${sunatResult.description}`;
      ...
      logAudit(
        ...
        'EMIT_DEBIT_NOTE',
        'DEBIT_NOTES',
        `Emitted ND ${serie}-${nextCorrelativo} (sync sendBill) affecting Factura ${affectedDoc.serie}-${affectedDoc.correlativo}. Status: ${documentRecord.status}`,
      ...
      FileDb.insert('notifications', {
        ...
        message: `Nota de Débito ${serie}-${nextCorrelativo} emitida. Pendiente de envío en Resumen Diario (RC).`,
        type: 'info',
        read: false,
      });
      ...
      logAudit(
        ...
        'EMIT_DEBIT_NOTE',
        'DEBIT_NOTES',
        `Emitted ND ${serie}-${nextCorrelativo} (async) affecting Boleta ${affectedDoc.serie}-${affectedDoc.correlativo}. Status: signed`,
        'success'
      );
      ...
      const response: NoteSignedResponse = {
        ...
        message: 'Nota de Débito firmada localmente. Lista para ser incluida en el Resumen Diario.',
      };
      return NextResponse.json(response);
    }
  } catch (error: any) {
